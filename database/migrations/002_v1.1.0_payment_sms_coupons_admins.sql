...
-- through phpMyAdmin (SQL tab). No data is deleted; only new columns/tables
-- are added.
--
-- Changes in this version:
--   1) Zarinpal payment integration -> new columns in orders
--   2) Checkout coupon system       -> coupon_id in orders
...
-- Existing orders (before this update) were recorded as "cash on delivery";
-- Mark them as paid so they remain consistent in payment reports
-- (the previous version had no online/offline payment concept). Remove this line if desired.
...
-- The first admin created by install.php is automatically promoted to super_admin
-- so it can create and manage additional admins from the panel.
